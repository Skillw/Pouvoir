...
open class Stored(title: String) : Basic(title) {
    ...
    /** 点击事件回调 仅在特定位置下触发 相比于 Basic，Stored 的所有点击事件回调均会处理 DRAG 类型 */
    ...
    /** 点击事件回调 仅在特定位置下触发 */
    override fun onClick(bindStr: String, callback: (event: ClickEvent) -> Unit) {
        val bind = bindStr[0]
        onClick {
            if (it.slot == bind) {
                callback(it)
    ...
    open class Rule {
        ...
        open fun checkSlot(callback: (inventory: Inventory, itemStack: ItemStack, slot: Int) -> Boolean) {
            val before = checkSlot
            checkSlot = { inventory, itemStack, slot ->
                callback(inventory, itemStack, slot) && before(
                    inventory,
                    itemStack,
                    slot
                )
        ...
        /** 获取页面中首个有效的位置 用于玩家 SHIFT 点击快速放入物品，不再触发 checkSlot 回调 */
