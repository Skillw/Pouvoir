...
import com.skillw.pouvoir.taboolib.module.ui.ClickType
import com.skillw.pouvoir.taboolib.module.ui.Menu
import com.skillw.pouvoir.taboolib.module.ui.MenuHolder
...
open class Basic(title: String = "chest") : Menu(title) {
    ...
    /** 行数 为 1 - 6 之间的整数，并非原版 9 的倍数 */
    open fun rows(rows: Int) {
        this.rows = rows
    }

    /**
     * 设置是否锁定玩家手部动作 设置为 true 则将阻止玩家在使用菜单时进行包括但不限于 丢弃物品，拿出菜单物品等行为
     *
     * @param handLocked 锁定
     */
    ...
    /** 页面关闭时触发回调 只能触发一次（玩家客户端强制关闭时会触发两次原版 InventoryCloseEvent 事件） */
    ...
    /** 点击事件回调 仅在特定位置下触发 */
    open fun onClick(bindStr: String, callback: (event: ClickEvent) -> Unit = {}) {
        val bind = bindStr[0]
        onClick {
            if (it.slot == bind) {
                it.isCancelled = true
    ...
    /** 根据抽象符号设置物品 */
    open fun set(slot: String, itemStack: ItemStack) {
        items[slot[0]] = itemStack
    }
    ...
    /** 根据抽象符号设置物品 */
    open fun set(slotStr: String, material: XMaterial, itemBuilder: ItemBuilder.() -> Unit = {}) {
        set(slotStr, buildItem(material, itemBuilder))
    }
    ...
    /** 根据抽象符号设置物品 */
    open fun set(slot: String, itemStack: ItemStack, onClick: ClickEvent.() -> Unit = {}) {
        set(slot, itemStack)
