...
open class Linked<T>(title: String) : Basic(title) {
    ...
    /** 元素生成回调 * */
    internal var generateCallback: ((player: Player, element: T, index: Int, slot: Int) -> ItemStack) =
        { _, _, _, _ -> ItemStack(Material.AIR) }

    /** 异步元素生成回调 * */
    internal var asyncGenerateCallback: ((player: Player, element: T, index: Int, slot: Int) -> ItemStack) =
        { _, _, _, _ -> ItemStack(Material.AIR) }
    ...
    open fun onGenerate(
        async: Boolean = false,
        callback: (player: Player, element: T, index: Int, slot: Int) -> ItemStack,
    ) {
        if (async) {
